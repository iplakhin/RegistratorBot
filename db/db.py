...
from db.models.models import User, Appointment, Timeslot
...
async def is_timeslot_available(appointment_date: datetime, timeslot_pk: int) -> bool:
    async with async_session() as session:
        result = await session.execute(
            select(Appointment).where(
                Appointment.appointment_date == appointment_date,
                Appointment.timeslot == timeslot_pk
            )
        )
        return result.scalar() is None

async def get_timeslot(weekday: int, start_time: str):
    async with async_session() as session:
        result = await session.execute(
...
async def get_available_timeslots(selected_date: datetime) -> List[Timeslot]:
    ...
    async with async_session() as session:
        ...
        booked_stmt = select(Appointment.timeslot_pk).where(Appointment.appointment_date == selected_date.date())
        booked_result = await session.execute(booked_stmt)
        booked_timeslot_ids = {row[0] for row in booked_result.all()}
...
async def get_timeslot_by_id(timeslot_id: int) -> Timeslot:
    async with async_session() as session:
        ...
        if timeslot:
            print("Получил таймслот: " + timeslot.scalar_one())
        else:
            print("Не получил нихуя")
